...
Original file is located at
    https://colab.research.google.com/drive/11F0K7pzot2Zq5Q7VSsdLovAfwDKgGzW1
"""
...
def decode(input_length, tagset, score):
    ...
    """
    viterbiVars = [[-math.inf]*len(tagset) for _ in range(input_length)]
    backmarkerVars = [[""]*len(tagset) for _ in range(input_length)]
    path = [[""]*len(tagset) for _ in range(input_length)]

    #compute score for the first tag
    for y in range(len(tagset)):
      viterbiVars[1][y] = (score(tagset[y],'<START>',1))
      backmarkerVars[1][y] = tagset[y]
      path[1][y] = '<START>' + ',' + tagset[y]

    # M * n^2
    for m in range(2,input_length-1):
      for y in range(len(tagset)):
        for x in range(len(tagset)):
          if (score(tagset[y],backmarkerVars[m-1][x],m)+viterbiVars[m-1][x]) > viterbiVars[m][y]: 
            viterbiVars[m][y] = score(tagset[y],backmarkerVars[m-1][x],m)+viterbiVars[m-1][x]
            backmarkerVars[m][y] = tagset[y]
            path[m][y] = path[m-1][x] + "," + tagset[y]

    # compute final path 
    for y in range(len(tagset)):
      if (score('<STOP>',backmarkerVars[input_length-2][y],input_length-1)+viterbiVars[input_length-2][y]) > viterbiVars[input_length-1][0]: 
        viterbiVars[input_length-1][0] = (score('<STOP>',backmarkerVars[input_length-2][y],input_length-1)+viterbiVars[input_length-2][y])
        backmarkerVars[input_length-1][0] = tagset[y]
        path[input_length-1][0] = path[input_length-2][y] + ',' + '<STOP>'

    return path[input_length-1][0].split(',')
